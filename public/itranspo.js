...
//so we can have multiple maps on the go in a non-mobile app
  ...
  Map = {
	...
	google.maps.event.addListener(map, 'zoom_changed', function() {
            if (map.zoom < 16) { alert("Stops not available at this zoom level. Please zoom in to load more stops.");
				 return; }
    ...
    addpoints : function() {
       ...
       $.get("/points/" + Map.Boundaries.swLat + "/" + Map.Boundaries.neLat + "/"
		...
		for(var key in result){
                    ...
                    var stopLatLng = new google.maps.LatLng(result[key]["stop"]["stop_lat"] 
				     - .00001  ,result[key]["stop"]["stop_lon"] + .0000455);
                    var marker = new google.maps.Marker ({
                    
                            position: stopLatLng,
                            map: map,
                            title: result[key]["stop"]["stop_name"],
			    icon: 'bus.png'
		      });
		    
		    var id = result[key]["stop"]["stop_id"]; //make a closure around this when adding click listener
